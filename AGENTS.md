...
| 全局状态 | `scripts/game_state.gd` | 货币、章节、升级、重置 |
| 全局事件 | `scripts/event_bus.gd` | 类型化信号中心 |
| 地图推进 | `scripts/map_manager.gd`, `scripts/floor_graph.gd`, `scripts/ui/map_screen.gd` | 路线选择与章节流程 |
...
| HUD 架构 | `scripts/ui/hud_root.gd`, `scripts/ui/hud_presenter.gd`, `scripts/ui/classic_header_layout.gd` | 分层 UI 架构 |
...
| MCP 运行时控制 | 外部 Godot MCP 工具 | 不提交 vendored addon；运行时由 MCP 工具临时注入控制桥 |
...
HUD 采用分层架构，数据流从 Presenter → 布局层 → Widget：

```
┌─────────────────────────────────────────────────────────────┐
│                     HudRoot (容器层)                        │
│  - 管理布局模式 (CLASSIC / MODERN)                          │
│  - 注入 Presenter 到子组件                                   │
│  - 协调布局显隐                                              │
│  - 不直接访问 GameState/WaveManager                          │
└───────────────────────┬─────────────────────────────────────┘
                        │
        ┌───────────────┴───────────────┐
        ▼                               ▼
┌───────────────────┐         ┌─────────────────────┐
│ ClassicHeaderLayout │         │ ModernHudLayout     │
│ (布局层)           │         │ (布局层)            │
│ - 顶部栏 UI        │         │ - 未来实现          │
│ - 包含 Widgets     │         └─────────────────────┘
└─────────┬─────────┘
...
└─────────┬─────────┘
    ...
    ▼           ▼               ▼
┌────────┐ ┌────────────┐ ┌──────────────────┐
│InputHints│ │SpeedIndicator│ │EnemyIndicatorLayer│
│Widget   │ │Widget       │ │                  │
└────────┘ └────────────┘ └──────────────────┘
...
│  - 收集展示数据                                              │
│  - 提供 get_xxx_state() 接口                                 │
│  - 监听 EventBus/MapManager，emit presentation_changed       │
│  - 不包含 UI 渲染逻辑                                        │
└─────────────────────────────────────────────────────────────┘
```

**关键原则：**
- 布局层不直接访问 GameState/WaveManager/MapManager
- 数据通过 Presenter 的 `get_xxx_state()` 接口获取
- 变更通知通过 `presentation_changed` 信号传递
...
- 不要在更多地方硬编码波次/章节映射——`wave_manager.gd` 已够脆弱
...
- Godot MCP 作为外部 QA 工具使用；不要把 `addons/godot_mcp` 重新 vendor 到项目内。
...
# 本环境下，首选运行时验证方式是外部 Godot MCP（按需临时注入运行时桥）
# 项目主路径：G:\dev\operation-taklamakan
```

## 备注

- 不要提交 `McpInteractionServer` autoload 或 `addons/godot_mcp`；MCP 运行时桥由工具在 QA 时临时注入。
